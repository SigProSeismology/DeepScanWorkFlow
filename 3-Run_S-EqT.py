...
def set_gpu_memory_fraction(gpu_fraction):
    ...
    if gpus:
        try:
            ...
            gpu_index = gpu_name.split(':')[-1]
            gpu_memory_info = tf.config.experimental.get_memory_info(f'GPU:{gpu_index}')
            print("Memory Info:", gpu_memory_info)  # Debugging
            gpu_memory = gpu_memory_info.get('total', 0)
            tf.config.set_logical_device_configuration(
                gpus[0],
                [tf.config.LogicalDeviceConfiguration(memory_limit=int(gpu_memory * gpu_fraction))])
