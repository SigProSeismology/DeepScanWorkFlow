...
def run_hyp(ztr):
    ...
    for line in lines:
        ...
        if line[0:5]=='CRE 1': line = "CRH 1 '%s' %s T \n"%(pmod, ref_ele)
        if line[0:5]=='CRE 2': line = "CRH 2 '%s' %s T \n"%(smod, ref_ele)
    ...
    fout.close()
    # 2. run hypoinverse
    p = subprocess.Popen(['/home/shazam/PycharmProjects/SEQTNEW/ESPRH/hyp1.40/bin/hyp1.40'], stdin=subprocess.PIPE,encoding='utf-8')
    s = "@{}".format(fhyp) + '\n'
    p.communicate(s)
